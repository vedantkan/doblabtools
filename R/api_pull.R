...
pull_redcap_data <- function(token = "", url = "", database){
  ...
  ### If database is consciousness/aphasia change record_id accordingly
  if(database == "consciousness"){
    df <- df %>% mutate(record_id = paste0("C-", record_id))
  }
  else if(database == "aphasia"){
    df <- df %>% mutate(record_id = paste0("A-", record_id))
  }
